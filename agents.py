...
def _sanitize_input(inp: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "packet_rate": _coerce_float(inp.get("packet_rate")),
        "protocol": str(inp.get("protocol", "UNKNOWN")).upper(),
        "flags_pattern": [str(f).upper() for f in inp.get("flags_pattern", [])],
        "connection_count": _coerce_int(inp.get("connection_count")),
        "batch_summary": str(inp.get("batch_summary", "")),
    }


def run_agents(payload: Dict[str, Any], llm_model="gemini/gemini-1.5-flash") -> Dict[str, Any]:
    clean_input = _sanitize_input(payload)

    # 🔥 SINGLE AGENT (fixes rate limit issue)
    agent = Agent(
        role="Network Security Analyst",
        goal="Analyze anomaly and suggest remediation in one step",
        backstory="Expert SOC analyst",
        llm=llm_model,
        verbose=False,
        allow_delegation=False,
        max_iter=1,  # 🔥 IMPORTANT (prevents retries)
    )

    task = Task(
        description=(
            "You are given network anomaly data:\n"
            f"{json.dumps(clean_input, indent=2)}\n\n"
    ...
    try:
        ...
        return {
            "analysis_input": clean_input,
            "analyzer_output": parsed.get("analyzer_output", {}),
            "remediation_output": parsed.get("remediation_output", {}),
        }
        ...
        return {
            ...
            "analyzer_output": {
                "anomaly_type": "unknown",
                "cause": "LLM failed (rate limit)",
                "confidence": "low",
                "evidence": [],
            },
            "remediation_output": {
                "recommended_actions": ["Retry after 60 seconds"],
                "priority": "medium",
                "notes": str(e),
            },
        }