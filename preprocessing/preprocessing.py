...
#     def safe_protocol_transform(self, protocols):
#         known_classes = set(self.protocol_encoder.classes_)
#         return np.array([
#             self.protocol_encoder.transform([p])[0] if p in known_classes else -1
#             for p in protocols
#         ])
...
#         else:
#             df['protocol_encoded'] = self.safe_protocol_transform(df['protocol'])
...
#         # # 🔥 FORCE TF-IDF USAGE
#         # info_text = df.get('Info', pd.Series([''] * len(df))).fillna('').astype(str)
...
#         # Encode labels
#         # Only encode labels during training
#         if fit and 'label' in df.columns:
...
class NetworkDataPreprocessor:
    ...
    def __init__(self):
        ...
        self.label_encoder = LabelEncoder()
        self.tfidf_vectorizer = TfidfVectorizer(max_features=300)
        self.flags_encoder = LabelEncoder()
        self.feature_columns: Optional[list] = None
        self.is_fitted = False
    ...
    def preprocess_data(self, dataset: pd.DataFrame, 
        ...
        if 'flags' in df.columns:
            ...
            df['flags_encoded'] = 0
        if 'flags' in df.columns:
            print("FLAGS SAMPLE:", df['flags'].head())
        print("FLAGS ENCODED:", df['flags_encoded'].head())
        # Select structured numeric features for ML
        self.feature_columns = [
            'packet_size',
            'packet_rate',
